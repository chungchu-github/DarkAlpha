...
def test_get_last_trigger_time_handles_naive_datetime_in_state(tmp_path) -> None:
    state_file = tmp_path / "risk_state.json"
    state_file.write_text(
        '{"days": {}, "last_trigger_by_symbol": {"BTCUSDT": "2026-02-25T12:00:00"}}',
        encoding="utf-8",
    )
    ...
    parsed = engine.get_last_trigger_time("BTCUSDT")

    assert parsed is not None
    assert parsed.tzinfo == timezone.utc


def test_evaluate_handles_naive_last_trigger_cooldown(tmp_path) -> None:
    state_file = tmp_path / "risk_state.json"
    state_file.write_text(
        '{"days": {}, "last_trigger_by_symbol": {"BTCUSDT": "2026-02-25T12:00:00"}}',
        encoding="utf-8",
    ...
    decision = engine.evaluate("BTCUSDT", now=datetime(2026, 2, 25, 12, 5, tzinfo=timezone.utc))
