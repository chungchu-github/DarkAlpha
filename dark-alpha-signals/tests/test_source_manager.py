...
class FakeRestClient:
    ...
    def fetch_premium_index(self, symbol: str):
        return 101.0, 0.0001, 1700000000000, datetime.now(tz=timezone.utc)
    ...
    def fetch_funding_rate_history(self, symbol: str, limit: int = 3):
        ...
        return [FundingRatePoint(funding_rate=0.0001, funding_time=1700000000000)], datetime.now(tz=timezone.utc)

    def fetch_open_interest(self, symbol: str):
        return 12345.0, datetime.now(tz=timezone.utc)
...
def _manager(datastore: DataStore, rest: FakeRestClient, ws: FakeWsClient) -> SourceManager:
    return SourceManager(
        ...
        premiumindex_poll_seconds=0.0,
        funding_poll_seconds=0.0,
        oi_poll_seconds=0.0,
    )
...
def test_derivatives_are_polled_and_stored() -> None:
    datastore = DataStore(symbols=["BTCUSDT"])
    manager = _manager(datastore, FakeRestClient(), FakeWsClient())

    manager.refresh()
    snapshot = datastore.snapshot("BTCUSDT")

    assert snapshot.mark_price == 101.0
    assert snapshot.last_funding_rate == 0.0001
